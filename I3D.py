...
#     # if use_batch_norm: net.add( GroupNormalization(filters//4))
#     if activation is not None: net.add( tf.keras.layers.Activation(activation) )
#     return net

class Unit3D(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size=(1, 1, 1), strides=(1, 1, 1), activation='relu', use_batch_norm=True, use_bias=False, name='unit_3d', **kwargs):
        super(Unit3D, self).__init__(name=name, **kwargs)
        self.use_batch_norm = use_batch_norm
        self.activation = activation
        self.conv3d = Conv3D(filters=filters, kernel_size=kernel_size, strides=strides, padding='SAME', use_bias=use_bias)
...
#     def __init__(self, filters, kernel_size=(1, 1, 1), strides=(1, 1, 1), activation='relu', use_batch_norm=True, use_bias=False, name='unit_3d'):
#         super(Unit3D, self).__init__()
#         self._name = name
#         self.use_batch_norm = use_batch_norm
#         self.conv3d = tf.keras.layers.Conv3D(filters=filters, kernel_size=kernel_size, strides=strides, padding='SAME', use_bias=use_bias)
...
class Inc(tf.keras.layers.Layer):
    def __init__(self, filters = [64, [96,128], [16,32], 32], name='Inc', **kwargs):
        ...
        self.branch_1_b = Unit3D(filters=filters[1][1], kernel_size=[3, 3, 3], name='Conv3d_0b_3x3')
        self.branch_2_a = Unit3D(filters=filters[2][0], kernel_size=[1, 1, 1], name='Conv3d_0a_1x1')
        self.branch_2_b = Unit3D(filters=filters[2][1], kernel_size=[3, 3, 3], name='Conv3d_0b_3x3')
        self.branch_3_a = MaxPool3D(pool_size=[3,3,3], strides=[1,1,1], padding='SAME', name='MaxPool3d_0a_3x3')
        self.branch_3_b = Unit3D(filters=filters[3], kernel_size=[1, 1, 1], name='Conv3d_0b_1x1')
...
def InceptionI3d(input_shape, num_classes=400, dropout_rate=0.1, name='inception_i3d'):
    ...
    with tf.name_scope('Logits'):
        net = AvgPool3D(pool_size=[2,7,7], strides=[1,1,1], padding='VALID')(net)
...
if __name__ == '__main__':
    ...
    # model.add(tf.keras.layers.InputLayer((3,10,10,64)))
